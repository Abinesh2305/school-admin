...
import 'dart:async';
import '../services/school_switcher_service.dart';
...
Future<void> main() async {
  ...
  runApp(SplashWrapper(savedTheme: savedTheme, savedLanguage: savedLanguage));
}
...
class SplashWrapper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MyApp(savedTheme: savedTheme, savedLanguage: savedLanguage);
  }
}
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      final displayTitle = senderName.isNotEmpty
          ? "$senderName – $title"
          : title;
  ...
  Future<void> resetFcmSubscriptions() async {
    ...
    await FirebaseMessaging.instance.unsubscribeFromTopic(
      "${AppConstants.topicPrefixSchool}$schoolId",
    );
    ...
    for (var u in uniqueUsers) {
      ...
      await FirebaseMessaging.instance.unsubscribeFromTopic(
        "${AppConstants.topicPrefixScholar}$uid",
      );
      await FirebaseMessaging.instance.unsubscribeFromTopic(
        "${AppConstants.topicPrefixSection}$sectionId",
      );
      ...
      for (var g in groups) {
        ...
        if (gid != null) {
          await FirebaseMessaging.instance.unsubscribeFromTopic(
            "${AppConstants.topicPrefixGroup}$gid",
          );
        }
      }
    }
    ...
    for (var u in uniqueUsers) {
      ...
      for (var g in groups) {
        final gid = g['id'];
        if (gid != null) {
          await safeSubscribe("${AppConstants.topicPrefixGroup}$gid");
        }
      }
    }
  }
  ...
  void _showGreetingOnAppOpen() {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      ...
      try {
        final greeting = GreetingHelper.getGreetingWithName(
          localizations,
          userName,
        );
  ...
  void _logoutUser() async {
    ...
    final firstLaunch = box.get(
      AppConstants.keyIsFirstLaunch,
      defaultValue: false,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic>? user = rawUser != null
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: TopNavBar(
          title: studentName,
          onHomeTap: () => _onNavigate(0),
          onToggleTheme: widget.onToggleTheme,
          onSwitchTap: () {
            SchoolSwitcherService.showSwitcher(context);
          },
        ),
